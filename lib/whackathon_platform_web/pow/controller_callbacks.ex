defmodule WhackathonPlatformWeb.Pow.ControllerCallbacks do
  @moduledoc false
  ...
  def before_respond(WhackathonPlatformWeb.SessionController, :create, {:ok, conn}, config) do
    ...
    ControllerCallbacks.before_respond(
      WhackathonPlatformWeb.SessionController,
      :create,
      {:ok, conn},
      config
    )
  ...
  def before_respond(WhackathonPlatformWeb.SessionController, :delete, {:ok, conn}, config) do
    live_socket_id = Conn.get_session(conn, @live_socket_id_key)
    WhackathonPlatformWeb.Endpoint.broadcast(live_socket_id, "disconnect", %{})
